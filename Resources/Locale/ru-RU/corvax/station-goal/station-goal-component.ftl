...
station-goal-start = [color=#b8972d]Orienta Corporation[/color]
    Orienta#XXX.3
...
station-goal-zoo =
    ...
    Для этого инженерный отдел должен построить зоопарк с как минимум 3 (тремя) вольерами для животных. На каждый вольер один вид. Каждый вольер должен быть обеспечен едой для конкретного вида и роботом-уборщиком. Площадь каждого вольера должна составлять минимум 16 м².
...
station-goal-security =
    { station-goal-start }[bold]
    Цель смены станции { $station } — постройка и снабжение тренировочного комплекса для KDS, с последующей проверкой состава.[/bold]
...
station-goal-shuttle-sec =
    ...
    2. Уметь совершать следующие движения: крен, тангаж и рысканье.
    3. Доступ к используемому оборудованию внутри отсеков не должен быть затруднён.
    4. Иметь на борту места и снаряжение как минимум на пять офицеров KDS.
...
station-goal-bunker =
    ...
    Необходимо пристроить по комнате, далее именуемой как “бункер”, следующим отделам: KDS, Медицинскому и Командному. В них экипаж станции мог бы укрыться от последствий гиперэнергетических потоков.
    ...
    Бункер отдела KDS должен так же включать в себя два Силовика и три Раздатчика летальных ружейных патронов.
...
station-goal-astra-repair =
    ...
    Вам необходимо силами утилизаторов и KDS при необходимости (по запросу утилизаторов) избавиться от мешающей проведению работ враждебной фауны.
